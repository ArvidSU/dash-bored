...
describe('migration detection and durable journey', () => {
  ...
  test('installation failures leave verified staging recoverable', async () => {
    const { dir, config } = await fixture(); const c = new UpdateCoordinator({ directory: dir, listDashboards: async () => [config], fetcher: fetcher(metadata('0.3.1')), install: async () => { throw new Error('installer unavailable'); } });
  ...
  test('invalid receipt and unselected dashboard never authorize edits', async () => {
    ...
    const c = new UpdateCoordinator({ directory: dir, listDashboards: async () => [], fetcher: fetcher(metadata('0.3.1')) });
