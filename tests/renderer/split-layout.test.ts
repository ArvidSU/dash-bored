...
describe("core tiled split sizing", () => {
  test("normalizes and clamps horizontal split ratios", () => {
  ...
  test("keeps vertical topology in document flow without pane scrollbars or separators", async () => {
    ...
    expect(styles).not.toContain("split--ratio-sized");
    expect(styles).not.toContain("split--content-sized");
  ...
  test("round-trips bounded per-dashboard overrides and rejects malformed entries", () => {
    ...
    expect(parseSplitRatioOverrides(JSON.stringify({
      ...
      legacyVertical: { ratio: 0.4, defaultRatio: 0.5, verticalSize: 320 },
      invalidRatio: { ratio: 2, defaultRatio: 0.5 },
    }))).toEqual({
      valid: { ratio: 0.4, defaultRatio: 0.5 },
      legacyVertical: { ratio: 0.4, defaultRatio: 0.5 },
    });
